...
export function AuctionPage() {
  ...
  if (!auctionId) return <Navigate to="/" replace />;
      ...
      <aside className="auction-page-account">
        {user ? (
          ...
          <div className="account-panel account-panel--guest">
            <div className="guest-panel-title">Want to bid?</div>
            <p className="guest-panel-body">Create a free account to place bids, deposit SOL, and win auctions.</p>
            <Link to="/register" className="btn-primary" style={{ display: 'block', textAlign: 'center', marginBottom: 10 }}>
              Create account
            </Link>
            <Link to="/login" className="btn-outline" style={{ display: 'block', textAlign: 'center' }}>
              Login
            </Link>
            <div className="guest-panel-hint">
              <Link to="/how-it-works">How do penny auctions work? →</Link>
            </div>
          </div>
        )}
      </aside>
